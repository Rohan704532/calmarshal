...
export default async function MeetingsRoute() {
    ...
    return (
        <>
            {data.data.length < 1 ? (
                <EmptyState title="No meetings found" description="You dont have any meeting yet." buttonText="Create a new event type" href="/dashboard/new" />
            ) : (
                <Card>
                    <CardHeader>
                        <CardTitle>Bookings</CardTitle>
                        <CardDescription>See upcoming events which where booked with an see the event type</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {data.data.map((item) => (
                                ...
                                <div className="grid grid-cols-3 justify-between items-center">
                                    <div>
                                        ...
                                        <div className="flex items-center mt-1">
                                            ...
                                            <a className="text-xs text-primary underline underline-offset-4" target='_blank' href={(item.conferencing as { details: { url: string } }).details.url}>Join Meeting</a>
                                        </div>
                                    </div>
